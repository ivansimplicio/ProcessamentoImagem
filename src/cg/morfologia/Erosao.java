...
/**
 * Implementa a operação morfológica erosão.
 * 
...
public class Erosao {
     ...
     * Construtor padrão.
     */
    public Erosao(int[][] imagem, int[][] elementoEstruturante, int width, int height) {
        this.imagem = imagem;
    ...
    /**
     * Aplica a operação morfológica erosão em cada pixel da imagem.
     */
    public BufferedImage run() {
        return Normalizacao.normalizaImage(aplicaErosao());
    }
    
    /**
     * Executa a erosao.
     */
    public int[][] aplicaErosao() {
        ...
        for (int i = 0; i < getHeight(); i++) {
            for (int j = 0; j < getWidth(); j++) {
                ...
                for(int cont = 0; cont < valores.length-1; cont++){
                    if(valores[cont] != 0 && valores [cont] < menor){
                        menor = valores[cont];
                    }
                }
                matrizImagem[i][j] = menor;
            }
        }
        return matrizImagem;
    }
